...
Site: Active24.cz
Docs: github.com/acmesh-official/acme.sh/wiki/dnsapi2#dns_active24
Options:
...
Author: trgo.sk <https://github.com/trgosk>, akulumbeg <https://github.com/akulumbeg>, litinoveweedle <https://github.com/litinoveweedle>
'

# Requirements: API Key and Secret from https://admin.active24.cz/en/auth/security-settings

ACTIVE24_Api="https://rest.active24.cz/api"

########  Public functions #####################

dns_active24_new_add() {
  fulldomain=$1
  txtvalue=$2
  ...
  if [ "$ACTIVE24_ApiKey" ] && [ "$ACTIVE24_ApiSecret" ]; then
    ...
    _err "You can get the API login credentials from https://admin.active24.cz/en/auth/security-settings"
    return 1
  fi
  ...
  #  if [ "$count" = "0" ]; then
  _info "Adding record"
  if _ACTIVE24_rest POST "/v1/user/self/zone/$_domain/record" "{\"type\":\"TXT\",\"name\":\"$_sub_domain\",\"content\":\"$txtvalue\",\"ttl\":120}"; then
...
dns_active24_new_rm() {
  ...
  _ACTIVE24_rest GET "/v1/user/self/zone/$_domain/record"
  ...
  if [ -z "$record_line" ]; then
    ...
    if ! _ACTIVE24_rest DELETE "/v1/user/self/zone/$_domain/record/$record_id"; then
...
_get_root() {
  ...
  while true; do
    ...
    if ! _ACTIVE24_rest GET "/v1/user/self/zone"; then
      return 1
    fi
...
_ACTIVE24_rest() {
  ...
  _debug2 api_key "$ACTIVE24_ApiKey"
  _debug2 api_secret "$ACTIVE24_ApiSecret"

  timestamp=$(_time)
  datez="$(_utc_date | sed "s/ /T/" | sed "s/$/+0000/")"
  canonical_request="${me} ${pa} ${timestamp}"
  signature_hash=$(printf "%s" "$canonical_request" | _hmac sha1 "$(printf "%s" "$ACTIVE24_ApiSecret" | _hex_dump | tr -d " ")" hex)
  basicauth="$(printf "%s:%s" "$ACTIVE24_ApiKey" "$signature_hash" | _base64)"
  ...
  if [ "$me" != "GET" ]; then
    _debug2 "${me} $ACTIVE24_Api${pa}"
    _debug data "$da"
    response="$(_post "$da" "${ACTIVE24_Api}${pa}" "" "$me")"
  else
    _debug2 "GET $ACTIVE24_Api${pa}"
    response="$(_get "$ACTIVE24_Api${pa}")"
  fi
